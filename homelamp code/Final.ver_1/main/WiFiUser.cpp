...
const char* mqtt_server = "mqtt.cetools.org";  // Your MQTT broker address
const int mqtt_port = 1884;                    // MQTT port (adjust if needed)
...
void Ledtrunon() {
  if (t1 < 500) {
    strip.setPixelColor(9, 80, 80, 80);
    strip.setPixelColor(8, 80, 80, 80);
    strip.setPixelColor(10, 80, 80, 80);
    strip.show();
  } else {
    strip.setPixelColor(8, 10, 10, 10);
    strip.setPixelColor(9, 10, 10, 10);
    strip.setPixelColor(10, 10, 10, 10);
    strip.show();
  }
  t1++;
}
